...
- [ ]  Add Console Minigames and PvP Content *`console minigames done (_CONSOLE_MINIGAMES on: Heavy Weapon; plus the last missing DS minigame, Air Raid) - rebuilt from behavioural descriptions with existing PC assets, not original console/DS assets, so they are faithful in feel rather than frame-accurate. Local Co-op partially done (_HAS_LOCAL_MULTIPLAYER on): same-machine 2 players on one shared lawn, player 2 on a gamepad (or arrow keys + Right Ctrl) driving a virtual cursor, with its own seed bank, own cooldowns and its own sun, toggled in More Settings. LOCAL ONLY - there is no netcode, no LAN and no sockets anywhere in this. Not yet done: the 11 dedicated console Co-op levels, the co-op double-sun pickup, a second seed chooser, and Versus mode (zombie-side brain economy, zombie placement UI and Target Zombies) which is deliberately unstarted`*
